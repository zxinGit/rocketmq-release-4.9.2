...
public class CommitLog {
    ...
    /**
     * 正常停止再启动，从倒数第三个文件开始恢复，如果不足三个，则从第一个开始恢复
     * checkCRCOnRecover 用于在进行文件恢复时查找消息是否验证CRC
    ...
    private boolean isMappedFileMatchedRecover(final MappedFile mappedFile) {
        ByteBuffer byteBuffer = mappedFile.sliceByteBuffer();
        //判断文件的魔数，false 表示该文件不符合commitLog的存储格式
        ...
        //文件的第一条消息存储时间为0，说明该文件未存储任何消息
        if (0 == storeTimestamp) {
            return false;
        }
        //对比第一条消息的时间戳和检测点的时间 如果比较小，说明该文件的部分消息是可靠的
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
       ...
       //如果不是事务消息类型或者是事务消息类型的提交类型
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
                || tranType == MessageSysFlag.TRANSACTION_COMMIT_TYPE) {
            // Delay Delivery
            //处理延迟消息
            if (msg.getDelayTimeLevel() > 0) {
                //如果设置的延时级别大于系统的默认最大级别，则重置为系统最大的延迟级别
                if (msg.getDelayTimeLevel() > this.defaultMessageStore.getScheduleMessageService().getMaxDelayLevel()) {
                    msg.setDelayTimeLevel(this.defaultMessageStore.getScheduleMessageService().getMaxDelayLevel());
                }
                //延时 队列的主题
                topic = TopicValidator.RMQ_SYS_SCHEDULE_TOPIC;
                //将延时级别转化为对应的延时队列的queueID
                queueId = ScheduleMessageService.delayLevel2QueueId(msg.getDelayTimeLevel());

                // Backup real topic, queueId
                //将原先的消息相关的属性放到延时队列的消息熟悉里面
        ...
        //发出主从同步请求，将请求提交到HaService进行数据复制
    ...
    /**
     * 返回当前文件的起始偏移量，如果mappedfile不可用，返回下一个文件的起始偏移量
     * @return
     */
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // 记录 ConsumeQueue 信息，获取该消息在消息队列额的物理偏移量
            ...
            //表示byteBuffer第一个字节存储的是消息的长度大小
            final int msgLen = preEncodeBuffer.getInt(0);

            // Determines whether there is sufficient free space
            //如果消息长度+END_FILE_MIN_BLANK_LENGTH 大于commitLog文件的空闲空间，返回END_OF_FILE，从这里可以看出
            //每个commitLog至少空闲8个字节，高四字节存储当前文件的剩余空间，底四字节存储魔数。
            ...
            final long beginTimeMills = CommitLog.this.defaultMessageStore.now();
            // Write messages to the queue buffer，写入buteBuffer
